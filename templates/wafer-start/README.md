...
- Build the product surface: replace the intentionally empty `src/routes/app.index.tsx`
...
- Follow the example feature wiring: `src/server/functions/todos.ts`, `src/server/services/todo-service.ts`, and `src/server/db/todo-repository.ts`
...
Examples and scaffolds in this starter:

- `src/routes/app.tsx`: shared `/app` shell and auth boundary
- `src/routes/app.index.tsx`: intentionally empty app index for agents to replace
...
The default `/app` page does not show a demo product UI. It is a quiet placeholder so scaffolded apps start clean. Database and Clerk scaffolding are already in place, and the todo implementation remains as an example feature path an agent can copy when the user asks for product behavior.
